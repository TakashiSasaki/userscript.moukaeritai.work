...
-   **MutationObserverの適切な利用**:
    ...
    -   **⚠️ 自己トリガー無限ループに注意**: コールバック内でDOMを変更すると、その変更が観測対象の子孫に含まれる場合、オブザーバーが再発火して無限ループになります。以下の対策を必ず組み合わせること:
        1. **状態フラグで更新を抑制**: 前回の状態と同じであればDOMを変更しない（例: `lastIsActive === isActive` なら早期リターン）。
        2. **観測スコープを絞る**: 自分が書き込むDOM要素（バッジ・トーストなど）が観測対象の外になるよう `subtree: true` を使わない、または別コンテナを使う。
        3. **デバウンスで吸収**: 上記2策と合わせてデバウンスを使い、仮に再発火しても連鎖を遮断する。
        4. **Navigation APIを活用**: SPAナビゲーション検知は `window.navigation.navigatesuccess` イベントで行い、MutationObserverへの依存を減らす。
...
-   **ルートの `index.html` のルール**:
    -   ルートディレクトリの `index.html` では、ユーザースクリプトごとの個別のカードを羅列せず、各ドメインディレクトリの `index.html` へのリンクを、ドメインの Favicon を表示しただけのシンプルなカード（`.domain-card`）として一覧表示する方針とします。
    -   新しいドメインを追加した場合は、必ずこのルートの `index.html` の `.domain-grid` 内にリンクを追加してください。
-   **ドメインディレクトリ以下のカードデザイン**:
    -   各ドメインディレクトリ以下の `index.html` では、引き続きスクリプトごとのカードを表示します。
    -   **タイトル**: サイトのFavicon (Google S2 API) + プロジェクト名（ドキュメントへのリンク）。
    -   **配置**: **インストールボタンはカードの右下(bottom-right)に配置**してください。
-   **バージョン更新**: ユーザースクリプト（`.user.js`）のコードを1行でも変更した場合は、必ず `@version` をインクリメントしてください。各 `index.html` 内のバージョン表記は `domain-landing.js` によって GitHub から動的に取得されるため、**HTML 内のバージョン番号を手動で書き換える必要はありません**。ただし、インストールボタンの `data-script-name` が正しいこと、および `href` が最新の GitHub Raw URL を指していることを確認してください。
