...
*   **Match URL**: `https://gemini.google.com/*` （スクリプト内部でURLを監視し、チャットページでのみ動的に有効化されます。現在、`/app/` および `/gem/` のパス形式に対応しています。）
...
*   **有効化**: チャットページ (`/app/...` または `/gem/...` ) に遷移した際に、エクスポートボタンやショートカット機能が自動的に有効になります。
...
## 更新履歴
*   **v0.1.22**: SPA対応を強化。`/app/` および `/gem/` の両方のURL形式に対応し、チャットページでのみ機能が動的に有効化・無効化されるように変更し、パフォーマンスを向上。
*   **v0.1.21**: `simulateClick` の `view` プロパティを `null` に変更し、Trusted Typesエラーを回避。
*   **v0.1.20**: キャンバスの `view-report-button` にエクスポートボタンを注入する機能を追加。
*   **v0.1.19**: キーボードショートカット (Ctrl+E) を実装。
*   **v0.1.18**: UI注入ロジックをリファクタリングし、各レスポンスの上部にもボタンを追加。
*   **v0.1.17**: モバイル版Geminiの「Export to...」の中間ボタンに対応。
*   **v0.1.16**: `waitForElement` の信頼性を向上させ、`simulateClick` に `mousedown`/`mouseup` イベントを追加。
