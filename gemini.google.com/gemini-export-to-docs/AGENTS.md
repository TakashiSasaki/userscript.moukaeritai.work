...
### Floating UI Panel (1-Turn Auto Export)
- **Draggable Header**: The panel's top header (marked with `⠿`) acts as a drag handle. Uses `mousedown`, `mousemove`, and `mouseup` on `document` to handle dragging.
