...
- **`section.studio-panel`**: 画面右端（チャットパネルのさらに右）。生成されたアーティファクト（メモや概要）を表示するパネル。これらはすべて `.panel-container` 内に並列で配置されています。
...
- **動的生成**: ソースリストやチャットメッセージは Angular により動的に生成されます。
- **監視対象**: `div.scroll-area-desktop` を `MutationObserver` の `childList` 監視対象とすることで、ソースの追加・削除を効率的に検知できます。