...
## 3. 自動化
各 `samples/` ディレクトリに `preprocess_samples.py` を配置し、これを使用して処理を自動化します。手動での編集は避け、スクリプトによる一貫した出力を維持してください。preprocess_samples.py は冪等性を持つこと。つまり処理済みのHTMLファイルを再度処理しても結果は同じであること。