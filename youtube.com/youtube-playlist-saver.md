...
### 3.4 補助機能
*   **自動スクロール**: 長いプレイリストの動画を効率的に読み込むため、画面右下に「Auto Scroll」ボタンを表示します。このボタンで自動スクロールの ON/OFF を切り替えることができます。
*   **プレイリストから削除**: 各動画のインジケーター横に「ゴミ箱」アイコンを表示します。これをクリックすると、YouTube のメニューを自動操作し、その動画を現在のプレイリストから削除します（データの保存状態には影響しません）。
*   **SPA 対応**: YouTube のシングルページアプリケーション遷移（`yt-navigate-finish` イベント等）を検知し、ページ移動時にもスクリプトが正しく再初期化されるようにします。
