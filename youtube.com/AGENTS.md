...
複数ユーザースクリプトが同時に注入されるため、初期化処理は 1〜3 秒のランダムディレイを入れてから実行すること。
- **狙い**: 初回ロード時の負荷集中を回避し、MutationObserver などの起動が同時に走らないようにする。
- **実装**: 一様分布で乱数を生成し、`setTimeout(init, delay)` で遅延実行する。固定値に戻さない。
