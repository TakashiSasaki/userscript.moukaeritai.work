...
# サムネイル画像へのマウスオーバー時の挙動

サムネイル画像にマウスオーバーした時に、
少し待ってから、
その画像を data URI にしたら何文字になるのかを表示する。
さらにクリックしたらそのdata URIをsrc属性に持つIMG要素を
ブラウザのクリップボードAPIを使ってクリップボードにコピーする
その際のMIMEは text/html とする。
さらに、クリップボードにコピー済みとしてサムネイル画像の縁を赤い線で囲む。

# 画像まとめてコピー機能

Images行に「Copy All」ボタンを配置し、クリックすると
表示されている全ての画像を `<img>` タグ形式で取得し、
まとめてクリップボードにコピーする (MIME: text/html)。
コピー完了時にはボタンの表示を「Copied!」などに一時的に変更してフィードバックを行う。
まとめて data URI を src にした複数のIMG要素を含む text/html としてクリップボードにコピーするボタンをつける。
